...
class customLabelDynamicLastProductOrder extends \ObjectModel
{
    ...
    public static $definition = [
        'table' => 'gmf_tags_dynamic_last_product_ordered',
        'primary' => 'id_tag',
        'fields' => [
    ...
    {
        return \Db::getInstance()->delete('gmf_tags_dynamic_last_product_ordered', 'id_tag=' . (int) $id_tag);
    }
    ...
    public static function getDynamicLastProductOrdered($id_tag)
    {
        $query = new \DbQuery();
        $query->select('*');
        $query->from('gmf_tags_dynamic_last_product_ordered', 'ftdlpo');
